...
def main():
    ...
    # Initializing centroid trackers with persistent tensors
    ...
    for epoch in range(args.epoch):
        ...
        for (image, image_label, mask, seg_idx) in tqdm(train_loader):
            ...
            with torch.cuda.amp.autocast():
                _, seg_patch_tokens, det_patch_tokens, seg_var, det_var, _, _ = model(image)           
                ...
                for layer in range(len(det_patch_tokens)):
                    ...
                    # Calculate centroid loss with gradients
                    intra_loss = 0
                    intra_inter_loss = 0
                    if len(normal_features) > 0:
                        # Use .data to access centroids without breaking graph
                        #intra_loss += torch.norm(normal_features - det_normal_centroids[layer].data.unsqueeze(0), p=2)
                        #intra_loss += (1-F.cosine_similarity(normal_features, det_normal_centroids[layer].data.unsqueeze(0), dim=-1)).mean()
                        intra_loss += torch.acos(torch.clamp(F.cosine_similarity(normal_features, det_normal_centroids[layer].data.unsqueeze(0), dim=-1), min=-1+1e-7, max=1-1e-7)).mean()
                        #intra_inter_loss += torch.norm(normal_features - det_anomaly_centroids[layer].data.unsqueeze(0), p=2)
                        #intra_inter_loss += F.cosine_similarity(normal_features, det_anomaly_centroids[layer].data.unsqueeze(0), dim=-1).mean()
                        intra_inter_loss += torch.acos(torch.clamp(F.cosine_similarity(normal_features, det_anomaly_centroids[layer].data.unsqueeze(0), dim=-1), min=-1+1e-7, max=1-1e-7)).mean()
                    if len(anomaly_features) > 0:
                        #intra_loss += torch.norm(anomaly_features - det_anomaly_centroids[layer].data.unsqueeze(0), p=2)
                        #intra_loss += (1-F.cosine_similarity(anomaly_features, det_anomaly_centroids[layer].data.unsqueeze(0), dim=-1)).mean()
                        intra_loss += torch.acos(torch.clamp(F.cosine_similarity(anomaly_features, det_anomaly_centroids[layer].data.unsqueeze(0), dim=-1), min=-1+1e-7, max=1-1e-7)).mean()
                        #intra_inter_loss += torch.norm(anomaly_features - det_normal_centroids[layer].data.unsqueeze(0), p=2)
                        #intra_inter_loss += F.cosine_similarity(anomaly_features, det_normal_centroids[layer].data.unsqueeze(0), dim=-1).mean()
                        intra_inter_loss += torch.acos(torch.clamp(F.cosine_similarity(anomaly_features, det_normal_centroids[layer].data.unsqueeze(0), dim=-1), min=-1+1e-7, max=1-1e-7)).mean()
                    ...
                    if len(normal_features) > 0:
                        if det_normal_centroids[layer].sum() == 0:
                            det_normal_centroids[layer].data.copy_(normal_features.squeeze(0))
                        else:
                            det_normal_centroids[layer].data.copy_(geodesic_update(det_normal_centroids[layer].data,normal_features.squeeze(0),0.5))
                    if len(anomaly_features) > 0:
                        if det_anomaly_centroids[layer].sum() == 0:
                            det_anomaly_centroids[layer].data.copy_(anomaly_features.squeeze(0))
                        else:
                            det_anomaly_centroids[layer].data.copy_(geodesic_update(det_anomaly_centroids[layer].data,anomaly_features.squeeze(0),0.5))
                ...
                if seg_idx > 0:
                    ...
                    for layer in range(len(seg_patch_tokens)):
                        ...
                        if len(normal_tokens) > 0:
                            #intra_loss_seg += torch.norm(normal_tokens - seg_normal_centroids[layer].data, p=2).mean()
                            #intra_loss_seg += (1-F.cosine_similarity(normal_tokens, seg_normal_centroids[layer].data, dim=-1)).mean()
                            intra_loss_seg += torch.acos(torch.clamp(F.cosine_similarity(normal_tokens, seg_normal_centroids[layer].data, dim=-1), min=-1+1e-7, max=1-1e-7)).mean()
                            #intra_inter_loss += torch.norm(normal_tokens - seg_anomaly_centroids[layer].data, p=2).mean()
                            #intra_inter_loss += F.cosine_similarity(normal_tokens, seg_anomaly_centroids[layer].data, dim=-1).mean()
                            intra_inter_loss += torch.acos(torch.clamp(F.cosine_similarity(normal_tokens, seg_anomaly_centroids[layer].data, dim=-1), min=-1+1e-7, max=1-1e-7)).mean()
                        if len(anomaly_tokens) > 0:
                            #intra_loss_seg += torch.norm(anomaly_tokens - seg_anomaly_centroids[layer].data, p=2).mean()
                            #intra_loss_seg += (1-F.cosine_similarity(anomaly_tokens, seg_anomaly_centroids[layer].data, dim=-1)).mean()
                            intra_loss_seg += torch.acos(torch.clamp(F.cosine_similarity(anomaly_tokens, seg_anomaly_centroids[layer].data, dim=-1), min=-1+1e-7, max=1-1e-7)).mean()
                            #intra_inter_loss += torch.norm(anomaly_tokens - seg_normal_centroids[layer].data, p=2).mean()
                            #intra_inter_loss += F.cosine_similarity(anomaly_tokens, seg_normal_centroids[layer].data, dim=-1).mean()
                            intra_inter_loss += torch.acos(torch.clamp(F.cosine_similarity(anomaly_tokens, seg_normal_centroids[layer].data, dim=-1), min=-1+1e-7, max=1-1e-7)).mean()
                        ...
                        if len(normal_tokens) > 0:
                            ...
                            if seg_normal_centroids[layer].sum() == 0:
                                seg_normal_centroids[layer].data.copy_(current_normal)
                            else:
                                seg_normal_centroids[layer].data.copy_(geodesic_update(seg_normal_centroids[layer].data,current_normal,0.5))

                        if len(anomaly_tokens) > 0:
                            current_anomaly = anomaly_tokens.mean(dim=0)
                            if seg_anomaly_centroids[layer].sum() == 0:
                                seg_anomaly_centroids[layer].data.copy_(current_anomaly)
                            else:
                                seg_anomaly_centroids[layer].data.copy_(geodesic_update(seg_anomaly_centroids[layer].data,current_anomaly,0.5))
