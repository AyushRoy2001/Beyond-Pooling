...
def encode_text_simple(model, obj, device):
    prompt_normal = f"a photo of a good {obj}"
    prompt_abnormal = f"a photo of a damaged {obj}"
    
    # Tokenize both prompts
    prompted_sentences = tokenize([prompt_normal, prompt_abnormal]).to(device)
    # Encode
    class_embeddings = model.encode_text(prompted_sentences)
    # Normalize each embedding separately
    class_embeddings = class_embeddings / (class_embeddings.norm(dim=-1, keepdim=True) + 1e-8)
    class_embeddings = class_embeddings.t()  # (768, 2)
    class_embeddings = class_embeddings.requires_grad_(True)   
